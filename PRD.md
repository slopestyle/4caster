...
| **Тип** | Единственный источник требований (Single Source of Truth) |

> **Правило работы с документом.** Любое расхождение между кодом и этим PRD трактуется как дефект.
> Изменение требований оформляется PR к этому файлу с ADR (Architecture Decision Record) в `docs/adr/`.
> Разделы, помеченные `[ASSUMPTION]`, требуют подтверждения до старта соответствующей фазы.
...
### Фаза 0 — Основание (Спринт 1, недели 1–2)
...
### Фаза 1 — Сбор данных и backfill (Спринт 2, недели 3–4)
...
**Milestone M1:** система непрерывно собирает данные и хранит их корректно.

### Фаза 2 — Ядро: консенсус и надёжность (Спринты 3–4, недели 5–8)
...
**Milestone M2:** консенсус и надёжность считаются для всех локаций каждые 4 часа.

### Фаза 3 — Верификация и обучение (Спринт 5, недели 9–10)
...
**Milestone M3:** система обучается на истории; Reliability откалиброван до релиза.

> **Ключевая зависимость:** задача 1.7 (backfill) обязана быть завершена до 3.4 — иначе обучать не на чем.

### Фаза 4 — Детекция изменений и уведомления (Спринт 6, недели 11–12)
...
**Milestone M4:** уведомления доставляются, спама нет (проверено симуляцией на истории).

### Фаза 5 — Telegram-интерфейс (Спринт 7, недели 13–14)
...
**Milestone M5 — Релиз MVP.**

### Фаза 6 — Стабилизация и закрытый бета-тест (недели 15–18)
...
| Фаза | Срок | Содержание | Ключевая метрика успеха |
|---|---|---|---|
| **MVP** | Q3 2026 | §4.2 | Actionable Alert Rate ≥ 60%, ECE ≤ 0.12 |
...
| **Expansion** | H2 2027 | Новые регионы, веб, монетизация | 5 000 активных пользователей |
