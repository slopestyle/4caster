...
2. **Telegram Mini App вытянут вперёд** из фазы Expansion — как основная поверхность продукта
   (список точек с поиском и группировкой по районам, недельный прогноз, метеограмма 48 ч,
   heatmap эволюции прогноза, разбор надёжности по компонентам §10.6).
...
| Reliability Score (§10.6) | калиброванный 0–100, 6 компонент | 🔄 компоненты A/E/S считаются в домене, σ_clim из архива, H — по априорной кривой §10.6.5; агрегация по горизонтам 1/3/7/14 §10.6.3 ✅ (наружу — только она); измеренная H, G/D и калибровка ⬜ (шкала, INV-6) |
