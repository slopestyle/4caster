...
> Обновляется при закрытии задач/фаз. Актуально на **26 июля 2026**.
...
|---|---|---|
| Локации | 31 (§7.3) | **2** (Ачишхо, Аибга; кластер CL-ALP-W) 🔄 |
| Модели | 5 детерм. + 2 ансамбля | **5 детерминированных** через Open-Meteo; ансамбли ⬜ |
| Downscaling (§10.3) | температура, изотерма, `k_oro` | ⬜ отложен |
| Reliability Score (§10.6) | калиброванный 0–100, 6 компонент | 🔄 предварительная надёжность по разбросу моделей; калибровка ⬜ |
| Accuracy Engine (§8.4, §10.8) | ground truth, метрики, обучение весов | ⬜ не начат (веса статические §8.2) |
...
|---|---|---|
| **0. Основание** | 🔄 | Каркас модулей, конфигурация, Alembic (3 миграции), CI (pytest) — ✅. Инфра пересмотрена на serverless ♻️ (0.5). Верификация координат 31 локации (0.3), наблюдаемость (0.6) — ⬜ |
| **1. Сбор данных** | 🔄 | Порт `ForecastProvider` + Open-Meteo ACL (1.1), планировщик 6 циклов на GitHub Actions (1.6) — ✅; нормализация (1.4) — 🔄. Rate limiter/circuit breaker (1.2), raw storage (1.3), downscaling (1.5), backfill (1.7), api.met.no (1.8) — ⬜ |
| **2. Ядро** | 🔄 | Взвешенные перцентили + POP (2.1), read-модель + проекция (2.8) — ✅; хранение истории (2.7), HIL (2.4) — 🔄. WeightResolver (2.2), корреляции/`N_eff` (2.3), Reliability A–G (2.5–2.6) — ⬜ |
| **3. Верификация и обучение** | ⬜ | Не начата — нет backfill, ground truth, обучения весов, калибровки |
...
1. **Расширить каталог локаций** (0.3) — верифицировать координаты/высоты и открыть остальные точки §7.3.
2. **Ансамбли + предварительный spread** → довести Reliability до вычисляемого вида (2.3, 2.5).
3. **Backfill архива** (1.7) — разблокирует Accuracy Engine и калибровку (Фаза 3).
4. **Downscaling** (1.5) — критично для точек > 2500 м (фаза осадков, изотерма).
...
> `[ASSUMPTION]` Координаты и высоты ниже — **предварительные, требуют верификации** по топокартам масштаба 1:50000 / SRTM+Copernicus DEM перед вводом в эксплуатацию. Колонка `Conf` — уверенность в координате: H(igh) / M(edium) / L(ow). Верификация — задача **Задачи 0.3** плана разработки; локации с `Conf = L` не публикуются пользователям до уточнения.
