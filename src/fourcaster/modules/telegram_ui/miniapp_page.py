...
график, дни, надёжность, история прогноза), сравнение точек.

Надёжность приходит готовой из read-модели — её считает домен (§10.6, компоненты
A/E/S) и сворачивает в горизонты 1/3/7/14 суток (§10.6.3). Показывается она
**на период целиком** («на ближайшие 3 дня»), а не на отдельные сутки внутри
него. Шкала качественная, а не число: калибровки по факту ещё нет (INV-6).
Расчёт по разбросу, оставшийся в JS ниже, — запасной путь для карточек,
записанных до появления доменного модуля; его можно убрать, когда в кэше не
останется старых.
"""
...
// Посуточная надёжность: её считает домен (§10.6) и кладёт в карточку — Mini App
// её только показывает (FR-TG-7). Пользователю эта величина как «надёжность» не
// показывается (для него есть горизонты §10.6.3): здесь она нужна вердикту
// «идти / не идти» на конкретный день. Оценка ниже — запасная, для карточек,
// записанных до появления доменного модуля.
...
// Разбор надёжности, посчитанной на сервере: компоненты §10.6 словами.
// Для горизонта это те же компоненты, усреднённые по всем его суткам.
function relParts(r){
  const pct=v=>Math.round(v*100)+'%';
  return `<div class="relnums">
      <div><span>согласие моделей</span><b>${pct(r.agreement)}</b></div>
      <div><span>разброс ансамблей</span><b>${r.ensemble_is_proxy?'не на все дни':pct(r.ensemble)}</b></div>
      <div><span>устойчивость прогноза</span><b>${r.stability==null?'мало истории':pct(r.stability)}</b></div>
      <div><span>оправдываемость срока</span><b>${r.history==null?'—':pct(r.history)}</b></div>
      <div><span>сценариев в расчёте</span><b>${r.n_members}</b></div>
    </div>
    <div class="note">Надёжность складывается из четырёх ответов, усреднённых по всем суткам
    периода. <b>Согласие моделей</b> — говорят ли независимые прогнозы одно и то же
    (и в миллиметрах, и по категории: 12 против 18 мм — это согласие, 0,1 против 2,5 —
    уже нет). <b>Разброс ансамблей</b> — насколько разошлись ${r.n_members} сценариев одной
    и той же модели, запущенных с чуть разными начальными данными${r.ensemble_is_proxy?'; на часть дней ансамбли не ответили, и оценка снижена':''}.
    <b>Устойчивость</b> — не скачет ли прогноз на эти даты от прогона к прогону: плавный
    тренд это норма, качели вверх-вниз — нет${r.stability==null?' (истории пока мало)':''}.
    <b>Оправдываемость срока</b> — насколько вообще сбываются прогнозы осадков на таком
    удалении: на десятые сутки модели тоже бывают дружны, но попадают в факт заметно реже
    (пока по типовой кривой для горного региона, не по нашим точкам).
    Провал по любому из четырёх обрушивает итог — иначе высокое согласие маскировало бы качели.</div>
...
// ── Надёжность на горизонт, а не на отдельный день (§10.6.3) ───────────────
// Пользователь идёт не «в четверг», а на сутки, выходные или неделю: вопрос
// звучит «на сколько дней вперёд этому прогнозу можно верить». Оценка горизонта
// приходит готовой из read-модели — это среднее геометрическое посуточных
// оценок за все его сутки, поэтому пара непредсказуемых дальних дней тянет
// вниз весь период.
const HZ=[1,3,7,14];
function hzWord(n){ return n===1 ? 'сутки' : `${n} ${plural(n,'день','дня','дней')}`; }
function hzSum(days,n,key){ return days.slice(0,n).reduce((s,x)=>s+x[key],0); }
// Запасной путь для карточек, записанных до появления горизонтов в домене:
// среднее посуточных уровней по тому же окну (округление вверх — в пользу
// осторожности). Убирается вместе с confLevel, когда старых карточек не останется.
function horizonsOf(d){
  if(d.horizons && d.horizons.length) return d.horizons;
  const days=d.days||[], out=[], seen=new Set();
  HZ.forEach(h=>{
    const n=Math.min(h,days.length); if(!n||seen.has(n)) return; seen.add(n);
    const win=days.slice(0,n), lv=win.map(x=>confLevel(x,x.n_models));
    const mean=lv.reduce((a,b)=>a+b,0)/n;
    out.push({days:n, start:win[0].day, end:win[n-1].day,
      level:Math.min(3,Math.ceil(mean-0.001)), label:CW[Math.min(3,Math.ceil(mean-0.001))],
      worst_day:win[lv.indexOf(Math.max(...lv))].day, fallback:true});
  });
  return out;
}
// Почему именно эти сутки просели: называем самую слабую компоненту §10.6,
// а не пересказываем разброс моделей. На дальнем дне модели часто дружны, и
// «спорить не о чем» рядом с «тянет период вниз» читалось как противоречие —
// проседает там срок, а не согласие.
const WEAK={
  agreement:'согласие моделей',
  ensemble:'разброс ансамблей: сценарии одной и той же модели разошлись широко',
  stability:'устойчивость: прогноз на эту дату скачет от прогона к прогону',
  history:'сам срок: дело не в моделях — прогнозы на такое удаление сбываются заметно реже'};
function weakReason(day){
  const r=day.reliability;
  if(!r) return relWhy(day).why;
  const c={agreement:r.agreement, ensemble:r.ensemble};
  if(r.history!=null) c.history=r.history;
  if(r.stability!=null) c.stability=r.stability;
  const k=Object.keys(c).reduce((a,b)=>c[b]<c[a]?b:a);
  // Про согласие моделей есть разбор на реальных числах — он полезнее общей фразы.
  return k==='agreement' ? relWhy(day).why
    : `Слабее всего здесь ${WEAK[k]} (${Math.round(c[k]*100)}%).`;
}
function showHorizon(i){
  const d=CACHE.fc; if(!d) return;
  const h=horizonsOf(d)[i]; if(!h) return;
  const win=(d.days||[]).slice(0,h.days);
  if(!win.length) return;
  const p50=hzSum(win,h.days,'p50'), p10=hzSum(win,h.days,'p10'), p90=hzSum(win,h.days,'p90');
  const wet=win.filter(x=>x.p50>=2).length;
  const worst=win.find(x=>x.day===h.worst_day)||win[win.length-1];
  // «Слабое звено» — сутки, которые тянут период вниз: по ним и объясняем,
  // в чём именно расходятся модели.
  const weak = h.days>1
    ? `<div class="mtxt" style="margin-top:8px">Тянет период вниз <b>${wd(worst.day)} ${dm(worst.day)}</b>.
       ${weakReason(worst)}</div>`
    : `<div class="mtxt" style="margin-top:8px">${weakReason(worst)}</div>`;
  const strip=win.map(x=>{const lv=confLevel(x,x.n_models);
    return `<span class="hbadge"><i class="dd ${CB[lv]}"></i>${wd(x.day)} ${dm(x.day)}<em>${g(x.p50)} мм</em></span>`;
  }).join('');
  openSheet(`<div class="sheet-hd"><h4>📊 Надёжность · ближайшие ${hzWord(h.days)}</h4>
    <button class="x" data-close aria-label="Закрыть">✕</button></div>
    <div class="mtop" style="margin:2px 0 8px"><b class="${GC[h.level]}">${CW[h.level]}</b>
      <span class="tiny">${LVL_DESC[h.level]}</span></div>
    <div class="mtxt">${h.days===1
      ? `Это оценка на <b>ближайшие сутки</b> (${wd(h.start)} ${dm(h.start)}) — самый надёжный участок прогноза.`
      : `Это оценка на <b>весь период ${dm(h.start)}–${dm(h.end)} целиком</b>, а не на отдельный
         день внутри него: в неё входят все ${h.days} ${plural(h.days,'сутки','суток','суток')},
         и самые непредсказуемые из них тянут её вниз.`}</div>
    ${weak}
    <div class="relnums">
      <div><span>период</span><b>${dm(h.start)} – ${dm(h.end)}</b></div>
      <div><span>суммарно за период</span><b>${g(p50)} мм</b></div>
      <div><span>от и до по моделям</span><b>${g(p10)}–${g(p90)} мм</b></div>
      <div><span>дней с дождём (от 2 мм)</span><b>${wet} из ${h.days}</b></div>
    </div>
    <div class="hike" style="margin:10px 0 2px">${strip}</div>`
    + (h.fallback ? `
    <div class="note">Оценка собрана в приложении по разбросу моделей: карточка записана
    до того, как надёжность на горизонт стали считать в конвейере. Следующий расчёт
    (каждые 4 часа) вернёт полную версию с разбором по компонентам.</div>` : relParts(h)));
}
// ── Хайкабельность: единый индикатор «идти / не идти» ───────────────────────
...
function relBlock(d){
  ...
  if(!hs.length) return '';
  // Чип — это ПЕРИОД («ближайшие 3 дня»), а не отдельные сутки: надёжность
  // даётся на всю дистанцию выхода целиком.
  const chips=hs.map((h,i)=>{
    const sum=hzSum(d.days,h.days,'p50');
    return `<button class="relchip" onclick="showHorizon(${i})"
        title="Ближайшие ${hzWord(h.days)} (${dm(h.start)}–${dm(h.end)}) целиком: суммарно ${g(sum)} мм — нажмите, чтобы разобрать">
      <div class="h">${hzWord(h.days).toUpperCase()}</div>
      <div class="dot ${CB[h.level]}"></div><div class="w ${GC[h.level]}">${CW[h.level]}</div>
      <div class="relchip-n">${g(sum)} мм суммарно</div></button>`;
  }).join('');
  ...
  const short=hs[0], long=hs[hs.length-1];
  // Без обещания монотонности: если завтрашний фронт спорный, а дальше сухо и
  // дружно, оценка на неделю может оказаться и выше суточной — так и скажем.
  const trend = hs.length>1
    ? `На ближайшие ${hzWord(short.days)} — <b class="${GC[short.level]}">${CW[short.level]}</b>,
       на ${hzWord(long.days)} целиком — <b class="${GC[long.level]}">${CW[long.level]}</b>.`
    : `На ближайшие ${hzWord(short.days)} — <b class="${GC[short.level]}">${CW[short.level]}</b>.`;
  return `<div class="rel"><h4>📊 Надёжность <span class="tiny" style="font-weight:400">— на период целиком, предварительно</span></h4>
    <div class="relrow">${chips}</div>
    <div class="relmeta">${trend} ${cover}${memNote}.
    Нажмите на любой период выше — разберём на реальных числах, почему именно такая оценка.
    ...
    <details class="rel-how"><summary>Как считается надёжность</summary>
      <div class="body">
        Оценка даётся <b>на период целиком</b>, а не на отдельный день: «ближайшие 3 дня»
        — это все трое суток сразу, и непредсказуемые сутки внутри периода тянут его вниз
        (свёртка геометрическая). Поэтому чем длиннее период, тем ниже оценка.
        Каждые сутки считают <b>5 независимых метеомоделей</b>${mem?` и два ансамбля —
        те же модели, запущенные ${mem} раз с чуть разными начальными данными`:''}, а сама
        посуточная оценка складывается из трёх вещей: насколько модели <b>согласны между
        собой</b>, насколько узок <b>разброс ансамблевых сценариев</b> и насколько прогноз
        на эту дату <b>устойчив от прогона к прогону</b>.
