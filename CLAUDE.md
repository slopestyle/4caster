...
- **Код — тонкий вертикальный срез, доросший до маленького живого продукта.**
  ...
  по единому пулу членов, надёжность A/E/S + H по априорной кривой с выдачей
  по горизонтам 1/3/7/14 суток, орографическая коррекция
...
src/fourcaster/
  ...
  modules/
    ...
    reliability/      score.py — компоненты A/E/S/H, свёртка (§10.6) и
                        агрегация по горизонтам 1/3/7/14 суток (§10.6.3),
...
`compute_reliability` (A/E/S/H, посуточно) → `aggregate_by_horizon` (1/3/7/14
суток, §10.6.3 — только это идёт наружу) → `classify_hil` → рендер карточки. С `--save`:
...
- **Надёжность считается в домене** ([reliability/](src/fourcaster/modules/reliability/)),
  ...
  останется старых.
- **Надёжность — на горизонт, а не на отдельные сутки (§10.6.3).** Пользователю
  показываются периоды «сутки / 3 дня / 7 дней / 14 дней целиком»; посуточная
  `Reliability` — промежуточный результат (её ещё использует индикатор
  хайкабельности в Mini App). Не возвращай посуточную оценку в UI как «надёжность
  на 3-й день»: это разные величины.
- **Убывание надёжности с горизонтом даёт компонента H, а не A/E/S.** Согласие
  моделей на дальнем сухом дне часто выше, чем на завтрашнем фронте; без H
  «неделя» выглядела бы надёжнее «суток». H пока берётся из априорной кривой
  §10.6.5 (`PRIOR_BSS`) — измеренная придёт из Accuracy Engine (Фаза 3).
  Монотонности по горизонтам это не гарантирует и не должно: если завтра спорно,
  а дальше сухо и дружно, оценка периода честно выше суточной.
